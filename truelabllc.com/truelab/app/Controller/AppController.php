...
//         'defaults' => 'php',
//         'timeout' => 3, // The session will timeout after 30 minutes of inactivity
...
//        $Email->send('My message');
        ...
        $userlogagencyname = $this->Auth->user('agencyname');
        $doctoruserlogname = $this->Auth->user('firstname');

        $this->set("loggedUserRole", $user_role);

        $this->set("userlogagencyname", $userlogagencyname);

        $this->set("doctoruserlogname", $doctoruserlogname);
        ...
        if ($this->Auth->user()) {
            ...
            $usernamedata = $userimage['User']['firstname'];

            $userroledata = $userimage['User']['role'];
            $useragencyname = $userimage['User']['agencyname'];
            $this->set('userimage', $userimagedata);

            $this->set('roledata', $userroledata);
            $this->set('usernamedata', $usernamedata);
        }
    }
...
////////////////////////////////////////////////////////////
    ...
    public function isAuthorized($user) {
        ...
        if (($this->params['prefix'] === 'admin') && ($user['role'] != 'admin') && ($user['role'] != 'client') && ($user['role'] != 'user')) {
